...
function syncAllConflicts() {
  ...
  console.log(`[sync] Conflict audit complete. ${conflictIdSet.size} event(s) conflicting:`,
              conflictIdSet.size ? [...conflictIdSet].join(', ') : 'none',
              `(${latency_us.toFixed(1)} µs)`);
...
async function handler(req, res) {
  ...
  if (method === 'POST' && url === '/api/undo') {
    ...
    console.log(`[undo] Restoring ${snapshot.length} event(s); stack depth now ${actionStack.length}`);
